...
\section{Real npm Update Scanner}

The project now includes a real npm scanner behind the mocked security concept. It downloads old and new npm tarballs into a temporary directory with a \texttt{lageh-npm-scan-} prefix, extracts them with path traversal and link protection, and never executes package code.

\begin{lstlisting}[language=Python,caption={Real npm scanner entry point}]
...
The scanner inspects changed files and package metadata for high-risk signals: new lifecycle scripts, \texttt{child\_process}, dynamic \texttt{eval}/\texttt{Function}, shell download commands, socket usage, credential environment access, and large encoded payloads. It can also scan local tarballs in tests without network access.

\begin{lstlisting}[language=Python,caption={Safe tarball extraction checks}]
